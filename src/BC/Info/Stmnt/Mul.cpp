...
namespace GDCC::BC
{
   ...
   void Info::addFunc_MuXU_W(Core::FastU n)
   {
      ...
      GDCC_BC_AddStmnt(Code::MuXU_W, 2, stk, lop[0], rop[0]);
      ...
      for(Core::FastU col = 1; col != colEnd; ++col)
      {
         ...
         for(; ro != rowEnd; --lo, ++ro)
         {
            GDCC_BC_AddStmnt(Code::MuXU_W, 2, stk, lop[lo], rop[ro]);
   ...
   void Info::addFunc_MulF_W(Core::FastU n)
   {
      ...
      // Clear operand signs.
      GDCC_BC_AddStmnt(Code::BAnd_W, 1, lop.hi, lop.hi, fi.maskExp | fi.maskMan);
      GDCC_BC_AddStmnt(Code::BAnd_W, 1, rop.hi, rop.hi, fi.maskExp | fi.maskMan);

      // Check for l being NaN before other special exponent checks.
      if(n > 1)
      {
         GDCC_BC_AddStmnt(Code::Move_W,    1, stk, lop[0]);
      ...
      GDCC_BC_AddStmnt(Code::BAnd_W, 1, stk, stk, fi.maskMan);
      GDCC_BC_AddStmnt(Code::BOrI_W, 1, stk, stk, fi.maskMan + 1);
      GDCC_BC_AddStmnt(Code::MuXU_W, n, Stk(2), stk, stk);

      // Discard the lower bits.
      GDCC_BC_AddStmnt(Code::ShRI_W, 1, Stk(n + 1), Stk(n + 1), fi.bitsMan);
      ...
      GDCC_BC_AddStmnt(Code::CmpI_GT_W, 1, stk, tmp.hi, fi.maskMan * 2 + 1);
      GDCC_BC_AddStmnt(Code::Jcnd_Nil,  1, stk, labelCapMan);

      GDCC_BC_AddStmnt(Code::ShRI_W, 1, tmp, tmp, 1);
   ...
   void Info::addFunc_MulK_W(Core::FastU n)
   {
      ...
      if(nm == n * 2)
      {
         GDCC_BC_AddStmnt(Code::MuXU_W, nm, stk, lop, rop);
   ...
   void Info::addFunc_MulU_W(Core::FastU n)
   {
      ...
      for(Core::FastU col = 1; col != n; ++col)
      {
         ...
         IR::Code    codeMul = sizeMul == 2 ? IR::Code::MuXU_W : IR::Code::MulU_W;
   ...
   void Info::addFunc_MulX_W(Core::FastU n)
   {
      ...
      GDCC_BC_AddStmnt(Code::Move_W, n, stk, rop);
      GDCC_BC_AddStmnt(Code::ShRI_W, 1, stk, rop.hi, 31);
      for(Core::FastU i = nf - 1; i--;)
         GDCC_BC_AddStmnt(Code::Copy_W, 1, stk, stk);
      ...
      if(fi.bitsF % 32)
      {
         GDCC_BC_AddStmnt(Code::ShRI_W, 1,  Stk(nm), Stk(nm), fi.bitsF);
         GDCC_BC_AddStmnt(Code::Move_W, nf, nul, stk);
