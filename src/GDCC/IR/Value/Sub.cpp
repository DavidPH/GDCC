   ...
   namespace IR
   {
      ...
      Type Type::PromoteSub(Type const &l, Type const &r)
      {
         ...
         if(l.t == TypeBase::Fixed && r.t == TypeBase::Point) return r.tPoint;
            return Type_Fixed(sizeof(FastU) * CHAR_BIT, 0, true, false);
      ...
      {
         Integ value = l.value; value -= r.value; value /= l.vtype.reprS;
         Type_Fixed type{sizeof(FastU) * CHAR_BIT, 0, true, false};
      ...
      Value_Point &operator -= (Value_Point &l, Value_Fixed const &r)
      {
         l.value -= number_cast<GDCC::FastU>(r.value >> r.vtype.bitsF) /
